...
### Fixed
- Fix infinite loop in follow-up scanner collection enumerator — `getNextMemoryCollection` wraps around, so the scanner looped forever until Cloud Run timeout killed it. Now detects wrap-around and stops after one full pass.

## [0.13.1] - 2026-03-09

### Fixed
- Add missing `is_user_organized` boolean to `SearchFiltersDto` — requests with this filter were rejected by NestJS validation, causing 500 errors on the organize feed
...
### Added
- **Curated Sort Endpoint (Memories)** — `POST /api/svc/v1/memories/by-curated`
  - Composite score combining rating, recency, significance, and relationship density
  - Supports optional `query` for hybrid search and `direction` for sort order
- **Curated Sort Endpoint (Spaces)** — `POST /api/svc/v1/spaces/by-curated`
  - Same curated scoring applied to space memories with full space filter support
- 2 new unit tests (191 total passing, 2 pre-existing failures)
- Bump `@prmichaelsen/remember-core` to latest
